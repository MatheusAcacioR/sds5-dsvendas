...
- No arquivo `application.properties` inserir os codigos de configuração .

- Setar as variaveis de ambiente de acordo com as informações abaixo e do banco que sera utilizado
```
...
- No diretorio `resources` criar o arquivo `import.sql` para poder inserir os codigos sql que irao popular o banco de dados 
...
## Criando a entidade vendedor 

- No package `com.sds.sds5` criar um subpackage de nome `entities` e dentro uma class `Seller` .

- Inserir os codigos para criação dos atributos da entidade Seller, bem como seu contructor, Geters e Seters. 

```java
package com.sds.sds5.entities;
...
## Criando e mapeando a entidade venda

- No package `com.sds.sds5.entities` criar uma class `Sale` .

- Inserir os codigos para criação dos atributos da entidade Sale, bem como seu contructor, Geters e Seters. 
...
```
## Rodando o banco em formato de teste

- Fazer uma copia do arquivo `application.properties` e nomear essa copia para `application-test.properties`

- Remover do arquivo de teste a primeira linha `spring.jpa.open-in-view=false` e do arquivo arquivo padrao de properties remover toda a configuração do banco de dados e adicionar a linha `spring.profiles.active==test` ficando assim os dois arquivos:
